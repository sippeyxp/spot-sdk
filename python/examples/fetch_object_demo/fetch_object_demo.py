...
def arm_object_grasp(config):
    ...
    lease = lease_client.take() #acquire()
    try:
        with bosdyn.client.lease.LeaseKeepAlive(lease_client):
            ...
            # Override object as stowable
            override_req = manipulation_api_pb2.ApiGraspOverrideRequest()
            override_req.carry_state_override.override_request = 3  # STOW and CARRY
            override_reply = manipulation_api_client.grasp_override_command(req)

            print("Override result: ", override_reply.header.error)

            if grasp_success:

                robot.logger.info('Grasp success. Start carry.')
                carry = RobotCommandBuilder.arm_ready_command()
                command_client.robot_command(carry)
                time.sleep(3.0)

                robot.logger.info('Start stow.')
